...
An audit on 2026-08-20 found 34 defects; adding a linter on 2026-08-21 found two more (F-35, a spec-documentation error; F-36, a live crash in CSV export). Of **36 known defects, 35 are fixed** (see [docs/ANALYSIS.md](docs/ANALYSIS.md)); `node tools/verify-findings.js` re-measures 19 of them programmatically against the live model. The one left — F-27, solver robustness in a capital-tight regime — is listed in [STATUS.md](STATUS.md); it isn't blocked on anything, just not urgent.
...
| **The demo scenario now works.** | It used to go insolvent in year 4 and default on $749,981 of senior debt. It now repays in full, stays solvent and reaches 97,744 toilets — about 1.8% of the target population, which is the honest headline. ([ADR-0013](docs/adr/0013-viable-default-scenario.md)) |
| **Micro-enterprises are financed for what they actually cost.** | The loan booked for each enterprise used to use setup cost alone — a seventh of what the fund's own affordability check said one enterprise needed once working capital is included. One `meCapitalRequirement()` function now decides both. Reach fell substantially as a direct result; one scenario (`with cost of capital`) flips from viable to insolvent. ([F-21](docs/ANALYSIS.md#f-21--me-growth-magic-numbers-and-inconsistent-startup-capital), [ADR-0031](docs/adr/0031-unify-me-capital-requirement.md)) |
...
68 tests in about 1 second (Node's built-in runner, Node ≥ 20; ESLint is the one devDependency — `npm ci` once to install it). Currently **68 pass, 0 todo, 0 fail**. Seven suites: golden (did any of 21 recorded scenarios move), invariants (16 ledger checks — is the ledger self-consistent), smoke (does the whole app actually run), startup (what does a user opening the page in a browser actually get, once the country fetch has run), wiring (does each input reach the model, and does it actually move the model's output), write-down (does realised loss on a written-down loan behave the way `MODEL_SPEC.md` says it does), and export (does clicking "Export CSV" actually produce a CSV instead of throwing — F-36).
...
Pre-1.0. The model core is structurally sound — pure, deterministic, with a cash identity that holds by construction — and is being corrected stage by stage against a written specification and a regression suite. Stages S0–S3 are complete. S4 (decision support) and S5 (structure) have not started. See [STATUS.md](STATUS.md).
